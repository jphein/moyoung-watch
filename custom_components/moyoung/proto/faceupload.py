...
def derive_face_index(count_before: Optional[int], count_after: Optional[int],
                      current_after: Optional[int], slot: int,
                      face_index: Optional[int] = None) -> int:
    """Pick the DISPLAY-LIST index for a just-uploaded custom face.

    SET_DISPLAY_WATCH_FACE selects by list index, which is NOT the storage slot — the custom
    face lands somewhere in the list depending on how many built-in faces precede it, so the
    old ``set_face(slot=6)`` selected a stock face. Priority:

      1. explicit ``face_index`` (caller override)
      2. list grew (``count_after > count_before``) -> the face was appended -> last index
      3. else the watch may have auto-activated it -> its post-upload ``current_after`` index
      4. else the last known index (``count_after - 1``)
      5. else the storage ``slot`` (legacy fallback)
    """
    if face_index is not None:
        return face_index
    if count_before is not None and count_after is not None and count_after > count_before:
        return max(0, count_after - 1)
...
async def upload(
    ...
    *,
    ...
    ``activate`` sends the legacy ``set_face(slot)`` selection as the final step. Pass
    ``activate=False`` when the caller will select the correct display-list index itself
    (the coordinator does this: the storage slot != the display-list index — see
    :func:`derive_face_index`).

    Returns True if the device acked transfer completion, False if we timed out waiting
    (the finish/set-transfer commands are still sent either way, matching DaFup behaviour).
    """
    if kind == "face":
